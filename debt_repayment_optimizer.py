    ...
    while extra_payment_funds > 0:
        ...
        if next_lowest_account:
            if next_lowest_account.balance <= extra_payment_funds:
                ...
                print(f"  Applying extra payment of ${extra_payment:.2f} to {next_lowest_account.name}")
                ...
                print(f"  Applying extra payment of ${extra_payment_funds:.2f} to {next_lowest_account.name}")
            ...
            for account in accounts:
                if account.name == account_name and amount > 0:
                    if account.balance <= amount:
                        print(f"  Applying added amount of ${account.balance:.2f} to {account.name}")
                        amount -= account.balance
                        account.balance = 0
                        account.closed = True
                    else:
                        account.balance -= amount
                        print(f"  Applying added amount of ${amount:.2f} to {account.name}")
                        amount = 0

    # Apply remaining added_amount to lowest balance accounts
    added_amount_remaining = added_amount - sum(added_amount_distribution.values()) if added_amount_distribution else added_amount
    for account in sorted(accounts, key=lambda x: x.balance):
        if added_amount_remaining > 0:
            if account.balance <= added_amount_remaining:
                ...
                print(f"  Applying added amount of ${added_amount_remaining:.2f} to {account.name}")
    ...
    for month in range(1, total_months + 1):
        ...
        for i, account in enumerate(accounts):
            if account.balance == 0 and not account.closed:
                print(f"  {account.name} is closed with an original payment of ${int(account.monthly_payment):.2f}")
                account.closed = True
        ...
        for i in closed_accounts:
            ...
            print(f"  Allocating ${payment:.2f} from {paid_off_account.name} to extra payment funds")
        
        extra_payment_funds += total_payment - actual_payment
        
        if extra_payment_funds > 0:
            print(f"Applying extra payment of ${extra_payment_funds:.2f} to next lowest balance account(s):")
            extra_payment_funds, applied_extra_payment = apply_extra_payment(accounts, extra_payment_funds)
            if not applied_extra_payment:
                print("  No accounts to apply extra payment to")
        
        for account in accounts:
            if account.balance > 0:
                print(f"  {account.name} remaining balance: ${account.balance:.2f}")
        
        print(f"Total payment for Month {month}: ${total_payment:.2f}")
        print(f"Extra payment funds: ${extra_payment_funds:.2f}")
        print()
